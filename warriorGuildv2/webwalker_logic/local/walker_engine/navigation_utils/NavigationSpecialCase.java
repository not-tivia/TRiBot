package scripts.warriorGuildv2.webwalker_logic.local.walker_engine.navigation_utils;
...
import scripts.warriorGuildv2.webwalker_logic.local.walker_engine.Loggable;
import scripts.warriorGuildv2.webwalker_logic.local.walker_engine.WaitFor;
import scripts.warriorGuildv2.webwalker_logic.local.walker_engine.interaction_handling.AccurateMouse;
import scripts.warriorGuildv2.webwalker_logic.local.walker_engine.interaction_handling.InteractionHelper;
import scripts.warriorGuildv2.webwalker_logic.local.walker_engine.interaction_handling.NPCInteraction;
import scripts.warriorGuildv2.webwalker_logic.shared.helpers.RSItemHelper;
import scripts.warriorGuildv2.webwalker_logic.shared.helpers.RSObjectHelper;
...
public class NavigationSpecialCase implements Loggable {
    ...
    public static boolean handle(SpecialLocation specialLocation){
        ...
        switch (specialLocation){
            ...
            case BEDABIN_CAMP:
            case POLLNIVNEACH:
                String carpetDestination = specialLocation == NavigationSpecialCase.SpecialLocation.SHANTAY_PASS ? "Shantay Pass" : specialLocation == NavigationSpecialCase.SpecialLocation.UZER ? "Uzer" : specialLocation == NavigationSpecialCase.SpecialLocation.BEDABIN_CAMP ? "Bedabin camp" : "Pollnivneach";
            ...
            case ZANARIS_RING:
                ...
                if (InteractionHelper.click(InteractionHelper.getRSObject(Filters.Objects.nameEquals("Door")), "Open", () -> NavigationSpecialCase.SpecialLocation.ZANARIS_RING.getRSTile().distanceTo(Player.getPosition()) < 5 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)){
                    return true;
                }
                break;
            case LUMBRIDGE_ZANARIS_SHED:
                if (InteractionHelper.click(InteractionHelper.getRSObject(Filters.Objects.nameEquals("Fairy ring")), "Use", () -> NavigationSpecialCase.SpecialLocation.LUMBRIDGE_ZANARIS_SHED.getRSTile().distanceTo(Player.getPosition()) < 5 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)){
                    return true;
                }
                break;
            ...
            case WATERFALL_DUNGEON_ENTRANCE:
                if (NavigationSpecialCase.SpecialLocation.WATERFALL_DUNGEON.getRSTile().distanceTo(Player.getPosition()) < 500){
                    return InteractionHelper.click(InteractionHelper.getRSObject(Filters.Objects.nameEquals("Door")), "Open", () -> NavigationSpecialCase.SpecialLocation.WATERFALL_DUNGEON_ENTRANCE.getRSTile().distanceTo(Player.getPosition()) < 5 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE);
            ...
            case WATERFALL_DUNGEON:
                if (InteractionHelper.click(InteractionHelper.getRSObject(Filters.Objects.idEquals(2010)), "Open", () -> Player.getPosition().getX() == NavigationSpecialCase.SpecialLocation.WATERFALL_DUNGEON.x ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE)){
                    return true;
                }
            ...
            case GNOME_WAYDAR:
                if (NPCInteraction.clickNpcAndWait(Filters.NPCs.nameEquals("Daero"), new String[]{"Travel"})){
                    if (WaitFor.condition(5000, () -> Player.getPosition().distanceTo(NavigationSpecialCase.SpecialLocation.GNOME_WAYDAR.getRSTile()) < 10 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) != WaitFor.Return.SUCCESS){
                        break;
                    }
            ...
            case CRASH_ISLAND:
                if (NPCInteraction.clickNpcAndWait(Filters.NPCs.nameEquals("Waydar"), new String[]{"Travel"})){
                    if (WaitFor.condition(5000, () -> Player.getPosition().distanceTo(NavigationSpecialCase.SpecialLocation.CRASH_ISLAND.getRSTile()) < 10 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) != WaitFor.Return.SUCCESS){
                        break;
                    }
                    WaitFor.milliseconds(1000, 2000);
                    return true;
            ...
            case APE_ATOLL_GLIDER_CRASH:
                if (NPCInteraction.clickNpcAndWait(Filters.NPCs.nameEquals("Lumdo"), new String[]{"Travel"})){
                    if (WaitFor.condition(5000, () -> Player.getPosition().distanceTo(NavigationSpecialCase.SpecialLocation.APE_ATOLL_GLIDER_CRASH.getRSTile()) < 10 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) != WaitFor.Return.SUCCESS){
                        break;
                    }
                    WaitFor.milliseconds(1000, 2000);
                    return true;
            ...
            case HAM_INSIDE:
                if (RSObjectHelper.exists(Filters.Objects.actionsContains("Pick-Lock"))){
                    ...
                    if (InteractionHelper.click(RSObjectHelper.get(Filters.Objects.actionsContains("Climb-down")), "Climb-down")){
                        WaitFor.condition(3000, () -> NavigationSpecialCase.SpecialLocation.HAM_INSIDE.getRSTile().distanceTo(Player.getPosition()) < 10 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE);
                        return true;
                    }
                }
                break;
